...
static int32_t pci_to_port_id(struct rte_pci_addr *pci_addr)
{
    ...
    for (port_id = 0; port_id < nr_eth_dev; port_id++) {
        ...
        if (ret != 0) {
            LSTACK_LOG(INFO, LSTACK, "Can't get dev info, port id:%u", port_id);
            continue;
        }
        pci_dev = RTE_DEV_TO_PCI(dev_info.device);
        if (rte_pci_addr_cmp(pci_addr, &pci_dev->addr) == 0) {
            break;
        }
        LSTACK_LOG(INFO, LSTACK, "probed NIC pci:%04x:%02x:%02x.%x, not match\n",
